...
window.executeStartSet = function() {
    ...
    // SECURITY FIX: Strip out book-mode anytime a student starts or resumes a set
...
window.toggleBookMode = function() {
    ...
    const qId = currentSlide.getAttribute('data-question-id');
    const isTeacher = window.currentUser && window.currentUser.role === 'teacher';
    const isAnswered = !!window.studentAnswers[qId];

    // SECURITY FIX: Prevent toggling book mode if the question is unanswered
...
window.triggerCelebration = function(setName) {
    ...
    // Find what set comes next
    const setIndex = window.orderedSets.indexOf(setName);
    const nextSet = window.orderedSets[setIndex + 1];
    
    // Dynamically update the button behavior depending on if there are more sets left
    if (nextSet) {
        ...
        celebBtn.onclick = () => {
            ...
            showHeadsUp(nextSet); // Boots directly into the next set's preparation page
        };
    } else {
...
function playNarration(slide) {
    ...
    if ('speechSynthesis' in window) {
        ...
        utterance1.onend = () => {
            // Initiate a strict 3-second pause before the second reading
            window.narrationTimeout = setTimeout(() => {
                // Failsafe: Ensures student hasn't navigated to a different slide while waiting
                ...
                utterance2.onend = () => {
                    // Only start timer after the SECOND reading finishes
        ...
        // Fallback for browsers that don't support Text-to-Speech
...
window.showSlide = function(targetIndex) {
    ...
    // SECURITY FIX: Strip out book-mode anytime a student hits a slide they haven't answered yet
    const currentQId = currentSlide.getAttribute('data-question-id');
    const isTeacher = window.currentUser && window.currentUser.role === 'teacher';
    const isAnswered = !!window.studentAnswers[currentQId];
...
window.nextSlide = function() {
    ...
    if (nextSlideNode) {
        const nextSet = nextSlideNode.getAttribute('data-set');
        
        // BOUNDARY CHECK: If sliding right crosses into a new set
        if (currentSet !== nextSet) {
            const status = getSetStatus(currentSet);
            // If the current set is fully answered, intercept the slide and throw the celebration/continue modal
            if (status.state === 'completed') {
                window.triggerCelebration(currentSet);
                return; // STOP the slide transition
            }
        }
        window.showSlide(window.currentSlideIndex + 1);
    } else {
        // Edge Case: Absolute end of the entire curriculum (no next slide exists)
        const status = getSetStatus(currentSet);
        if (status.state === 'completed') {
            window.triggerCelebration(currentSet);
        }
        ...
        setTimeout(() => {
         ...
         setTimeout(() => {
             if(window.currentSlideIndex < window.slides.length - 1) {
                 window.nextSlide(); // Uses nextSlide instead of showSlide to properly obey boundary physics
             }
         }, 3000);
    }
};
